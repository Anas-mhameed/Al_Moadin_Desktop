...
class StopDialogSignal(QObject):
    
    stop_dialog = Signal(object)

    def stick(self, func):
        self.stop_dialog.connect(func)

    def send(self, dialog):
        self.stop_dialog.emit(dialog)
...
class NotificationManager:
    ...
    def create_notification(self, noti_type, is_permenant, *args):
        ...
        new_notification = AdanNotification(noti_type, time, adan_index, minutes, file_path, date, duration)
        ...
        self.save_notification_in_db((adan_index, minutes, date_in_db, duration, file_path, is_permenant, noti_type, 1))
    ...
    def update_notification_in_db(self, notification, new_file_path):
        def temp(func):
            if func():
                self.database_manager.update_notification(notification.get_index(), notification.get_minutes(), new_file_path)
        runnable = Runnable(temp)
        self.runnable_manager.runTask(runnable)
    ...
    def connect_noti_to_ui(self, new_notification):
        ...
        new_notification.attach_ui(ui_obj, self.update_notification_in_db, self.destroy_notification, self.change_noti_state_in_db, self.main_window)
    ...
    def intialize_notification(self, is_permenant, noti_type, adan_index, minutes, date, duration, file_path, is_active):
    ...
    def update_notis_and_intiate_index(self):
    ...
    def get_adan_time(self, adan_index):
        return self.player_manager.get_adan_time_from_manager(adan_index) 
    ...
    def compare_datetimes(self, curr_time, noti):
        return noti.get_adjusted_datetime() - curr_time.get_curr_time()
